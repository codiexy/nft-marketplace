...
export class MongoDbClient {
    ...
    filterMongoDbUrl = (str: any) => {
        const newStr: string = typeof str === "string" && str.trim() ? str.trim() : (env.MONGODB_URI || "mongodb://localhost:27017");
        if (!newStr) {
    ...
    filterMongoDatabase = (str: any) => {
        const newStr: string = typeof str === "string" && str.trim() ? str.trim() : (env.MONGODB_NAME || "diamond-verse");
        if (!newStr) {
