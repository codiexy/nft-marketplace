...
import { Button, CircularProgress, FormControl, Grid, Typography, TextField } from "@mui/material";
...
import { getTransactionOptions, subString, toCaptalize } from "helpers";
...
const BuyerMarketPlaceComponent: FC<MarketplaceComponentProps> = ({ nft, useStates, useModalOpen, setError, useUpdate }) => {
    ...
    const [formData, setFormData] = useState<FormDataProps>({
        ...
        endDate: new Date(moment().add(1, 'day').format("DD-MM-YYYY, 23:59:00"))
    });
    ...
    const handleModal = (slug: string = "") => {
        ...
        if (slug !== "bid") {
            setFormData({
                ...formData,
                endDate: new Date(moment().add(1, 'day').format('YYYY-MM-DD'))
            })
        }
    }
    ...
    const handlePlaceAuctionBid = async (event: any) => {
        ...
        try {
            ...
            setError(errorData.slug, errorData.message);
        }
    }
    ...
    const placeABid = async () => {
        try {
            ...
            const marketPlaceTransaction = await marketplaceContract.bidPlace(
                // parseInt(itemId),
    ...
    const placeABidAPI = async (tx: any = {}) => {
        try {
            ...
            let errorData = formatSolidityError(error.message);
            if (!errorData?.slug) {
                errorData.slug = "onOwnServer|auctioning";
            }
            throw new Error(JSON.stringify(errorData));
        }
    ...
    const makeAOfferForNft = async () => {
        try {
            ...
            const marketPlaceTransaction = await marketplaceContract.createOfferForItem(
                // parseInt(itemId),
                parseInt(marketplace.actionId),
                price,
                moment().utc().add(5, 'minute').unix(),
    ...
    const offerNftItemAPI = async (tx: any = {}) => {
        ...
        try {
            ...
            let errorData = formatSolidityError(error.message);
            if (!errorData?.slug) {
                errorData.slug = "onOwnServer|offering";
            }
            throw new Error(JSON.stringify(errorData));
        }
        ...
        try {
            Object.keys(modal).forEach(async (element) => {
                try {
                    ...
                    let errorData = formatSolidityError(error.message);
                    if (!errorData?.slug) {
                        errorData.slug = activeModal?.slice(0, -3) || "";
                    }
                    setError(errorData.slug, errorData.message);
                }
            });
        ...
        <>
                                                ...
                                                <div className="view-proof-authenticity-section dark:text-[#fff] text-[#000] text-xs font-bold flex">
                                                    <p className="grid grid-cols-12 gap-2">
                                                        <span className="col-span-10 text-xl ml-2">
                                                            <EthIcon>
                                                                {parseFloat(parseFloat(nft?.price?.eth || "0").toFixed(4))}
                                                            </EthIcon>
                                                            <span className="text-xs ml-2">(
                                                                <DollarIcon>
                                                                    {parseFloat(parseFloat(nft?.price?.dollar || "0").toFixed(4))}
                                                                </DollarIcon>
                                                                )</span>
                                                        </span>
                                    ...
                                    case 'open_for_bids':
                                        return <div className="open-for-bids-section dark:text-white py-3">
                                            ...
                                            <div className="grid grid-cols-2 gap-2" >
                                                <div className="view-proof-authenticity-section dark:text-[#fff] text-[#000] text-xs font-bold flex">
                                                    {
                                                        parseFloat(nft.price?.eth || "0") > 0 ? (
                                                            <p className="grid grid-cols-12 gap-2">
                                                                <span className="col-span-10 text-xl ml-2">
                                                                    <EthIcon>
                                                                        {parseFloat(parseFloat(nft.price.eth).toFixed(4))}
                                                                    </EthIcon>
                                                                    <span className="text-xs ml-2">
                                                                        (<DollarIcon>
                                                                            {parseFloat(parseFloat(nft.price.dollar).toFixed(4))}
                                                                        </DollarIcon>)
                                                                    </span>
                                                                </span>
                                                                <span className="text-xs"></span>
                                            ...
                                            <div className=" my-5 nftdetail_bids  singlepage_owner_list flex ">
                                                <div className="nftdetail_bids_col">
                                                    <div className="ml-2 items-center">
                                                        ...
                                                        <h5 className=" nftdetail_price_row_price  dark:text-[#fff] text-[#707a83] text-xs flex">
                                                            <EthIcon>
                                                                {parseFloat(parseFloat(currentBidData?.maxBidPrice?.eth || "0").toFixed(4))}
                                                            </EthIcon>
                                                            <span className="text-xs ml-2">(
                                                                <DollarIcon>
                                                                    {parseFloat(parseFloat(currentBidData?.maxBidPrice?.dollar || "0").toFixed(4))}
                                                                </DollarIcon>
                                                                )</span>
                                                        </h5>
                                                    </div>
                                                ...
                                                <div className="nftdetail_bids_col">
                                                    <div className="ml-2 items-center">
                                                        ...
                                                        <h5 className=" nftdetail_price_row_price  dark:text-[#fff] text-[#707a83] text-xs flex">
                                                            <EthIcon>
                                                                {parseFloat(parseFloat(currentBidData?.initialBidPrice?.eth || "0").toFixed(4))}
                                                            </EthIcon>
                                                            <span className="text-xs ml-2">(
                                                                <DollarIcon>
                                                                    {parseFloat(parseFloat(currentBidData?.initialBidPrice?.dollar || "0").toFixed(4))}
                                                                </DollarIcon>
                                                                )</span>
                                                        </h5>
                                                    </div>
            ...
            >
                <ModalHeader onClose={() => resetModal()}>
                    <span className="font-bold">{modalData.slug === "offer" ? "Make Offer" : "Place A Bid"}</span>
                </ModalHeader>
                <ModalContent>
                    {
                                    ...
                                    >
                                        ...
                                        <LocalizationProvider dateAdapter={AdapterDateFns}>
                                            <DatePicker
                                                ...
                                                onChange={(newValue) => {
                                                    const endDate = newValue ? new Date(newValue) : new Date(moment(formData.startDate).add(1, 'day').format("DD-MM-YYYY, 23:59:00"))
                                                    setFormData({
                                                        ...formData,
                                                        endDate
                                                    });
...
const OwnerMarketPlaceComponent: FC<MarketplaceComponentProps> = ({ nft, useStates, useModalOpen, setError, useUpdate }) => {
    ...
    const acceptOfferNftAPI = async (offerId = "", _offerer = "", tx: any = {}) => {
        try {
            ...
            let errorData = formatSolidityError(error.message);
            if (!errorData?.slug) {
                errorData.slug = "onOwnServer|acceptOffering";
            }
            throw new Error(JSON.stringify(errorData));
        }
    ...
    const tranferNftApi = async (tx: any = {}) => {
        ...
        try {
            ...
            let errorData = formatSolidityError(error.message);
            if (!errorData?.slug) {
                errorData.slug = "onOwnServer|transfering";
            }
            throw new Error(JSON.stringify(errorData));
        }
                                                            ...
                                                            <tbody className="bg-white dark:bg-slate-800">
                                                                {
                                                                    currentOffers.length ? (
                                                                        currentOffers.map((offer: any, key: number) => {
                                                                            ...
                                                                            return (
                                                                                <tr key={key}>
                                                                                    <td className="border-b border-slate-100 dark:border-slate-700 py-4 px-2 text-slate-500 dark:text-slate-400"><EthIcon>{parseFloat(parseFloat(offer?.offerPrice?.eth || "0").toFixed(4))}</EthIcon></td>
                                                                                    <td className="border-b border-slate-100 dark:border-slate-700 py-4 px-2 text-slate-500 dark:text-slate-400"><DollarIcon>{parseFloat(parseFloat(offer?.offerPrice?.dollar || "0").toFixed(4))}</DollarIcon></td>
                                                ...
                                                <div className=" my-5 nftdetail_bids  singlepage_owner_list flex" style={{ border: 'none' }}>
                                                    <div className="nftdetail_bids_col">
                                                        <div className="ml-2 items-center">
                                                            ...
                                                            <h5 className=" nftdetail_price_row_price  dark:text-[#fff] text-[#707a83] text-xs flex">
                                                                <EthIcon>
                                                                    {parseFloat(parseFloat(currentAuction?.maxBidPrice?.eth || "0").toFixed(4))}
                                                                </EthIcon>
                                                                <span className="text-xs ml-2">(
                                                                    <DollarIcon>
                                                                        {parseFloat(parseFloat(currentAuction?.maxBidPrice?.dollar || "0").toFixed(4))}
                                                                    </DollarIcon>
                                                                    )</span>
                                                            </h5>
                                                        </div>
                                                    ...
                                                    <div className="nftdetail_bids_col">
                                                        <div className="ml-2 items-center">
                                                            ...
                                                            <h5 className=" nftdetail_price_row_price  dark:text-[#fff] text-[#707a83] text-xs flex">
                                                                <EthIcon>
                                                                    {parseFloat(parseFloat(currentAuction?.initialBidPrice?.eth || "0").toFixed(4))}
                                                                </EthIcon>
                                                                <span className="text-xs ml-2">(
                                                                    <DollarIcon>
                                                                        {parseFloat(parseFloat(currentAuction?.initialBidPrice?.dollar || "0").toFixed(4))}
                                                                    </DollarIcon>
                                                                    )</span>
                                                            </h5>
                                                        </div>
                                                                            ...
                                                                            return (
                                                                                <tr key={key}>
                                                                                    <td className="border-b border-slate-100 dark:border-slate-700 py-4 px-2 text-slate-500 dark:text-slate-400">
                                                                                        <EthIcon>
                                                                                            {parseFloat(parseFloat(auctionHistory?.initialBidPrice?.eth || "0").toFixed(4))}
                                                                                        </EthIcon>
                                                                                        <span className="text-xs ml-2">(
                                                                                            <DollarIcon>
                                                                                                {parseFloat(parseFloat(auctionHistory?.initialBidPrice?.dollar || "0").toFixed(4))}
                                                                                            </DollarIcon>
                                                                                            )</span>
                                                                                    </td>
                                                                                    <td className="border-b border-slate-100 dark:border-slate-700 py-4 px-2 text-slate-500 dark:text-slate-400">
                                                                                        <EthIcon>
                                                                                            {parseFloat(parseFloat(auctionHistory?.minBidPrice?.eth || "0").toFixed(4))}
                                                                                        </EthIcon>
                                                                                        <span className="text-xs ml-2">(
                                                                                            <DollarIcon>
                                                                                                {parseFloat(parseFloat(auctionHistory?.minBidPrice?.dollar || "0").toFixed(4))}
                                                                                            </DollarIcon>
                                                                                            )</span>
                                                                                    </td>
                                                                                    <td className="border-b border-slate-100 dark:border-slate-700 py-4 px-2 text-slate-500 dark:text-slate-400">
                                                                                        <EthIcon>
                                                                                            {parseFloat(parseFloat(auctionHistory?.maxBidPrice?.eth || "0").toFixed(4))}
                                                                                        </EthIcon>
                                                                                        <span className="text-xs ml-2">(
                                                                                            <DollarIcon>
                                                                                                {parseFloat(parseFloat(auctionHistory?.maxBidPrice?.dollar || "0").toFixed(4))}
                                                                                            </DollarIcon>
                                                                                            )</span>
                                                                                    </td>
                                                                                    <td className="border-b border-slate-200 dark:border-slate-600 py-4 px-2 text-slate-500 dark:text-slate-400">
